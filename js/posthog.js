...
// Global listener for automatic click tracking with context metadata
document.addEventListener('click', function(e) {
    const target = e.target.closest('button, .person, .stat, a, input, label');
    if (target) {
        const text = (target.innerText || target.value || target.getAttribute('title') || '').trim().substring(0, 50);
        const tag = target.tagName.toLowerCase();
        const id = target.id || target.className || '';
        trackEvent('click_ui_element', {
            element_text: text,
            element_tag: tag,
            element_id: id,
            screen_name: typeof activeScreen !== 'undefined' ? activeScreen : 'setup'
        });
    }
});